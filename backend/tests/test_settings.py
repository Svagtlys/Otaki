...
async def test_patch_calls_write_env(logged_in_client, monkeypatch):
    ...
    await logged_in_client.patch(
        "/api/settings", json={"chapter_naming_format": "{title}.cbz"}
    )
...
async def test_patch_invalid_directory_returns_400(logged_in_client, monkeypatch):
    ...
    r = await logged_in_client.patch(
        "/api/settings", json={"library_path": "/nonexistent/path"}
    )
    assert r.status_code == 400
    assert "library_path" in r.json()["detail"]


async def test_patch_no_ping_when_suwayomi_fields_not_changed(
