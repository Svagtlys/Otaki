...
async def _add_source(
    *, name="Src", suwayomi_source_id="src-bk-1", priority=1
) -> Source:
...
async def test_export_active_only_by_default(logged_in_client):
    ...
    await _add_assignment(
        ...
        chapter_number=1.0,
        is_active=True,
        suwayomi_chapter_id="ch-act-active",
    )
    await _add_assignment(
        comic.id,
        source.id,
        chapter_number=1.0,
        is_active=False,
        suwayomi_chapter_id="ch-act-inactive",
    )
...
async def test_export_include_all_assignments(logged_in_client):
    ...
    await _add_assignment(
        ...
        chapter_number=1.0,
        is_active=True,
        suwayomi_chapter_id="ch-all-active",
    )
    await _add_assignment(
        comic.id,
        source.id,
        chapter_number=1.0,
        is_active=False,
        suwayomi_chapter_id="ch-all-inactive",
    )

    r = await logged_in_client.get(
        "/api/settings/export?format=json&include_all_assignments=true"
    )
    data = r.json()
    chapter_ids = [ch["suwayomi_chapter_id"] for ch in data["chapter_assignments"]]
...
async def test_preview_returns_new_comics(logged_in_client):
    await _add_source(suwayomi_source_id="src-prev-new")
    backup = {
        "version": 1,
        "sources": [
            {
                "_id": 1,
                "suwayomi_source_id": "src-prev-new",
                "name": "Src",
                "priority": 1,
                "enabled": True,
            }
        ...
        "comics": [
            {
                "_id": 1,
                "title": "Brand New Comic",
                "library_title": "Brand New Comic",
                "status": "tracking",
                "poll_override_days": None,
        ...
        "chapter_assignments": [
            {
                ...
                "suwayomi_chapter_id": "ch-prev-1",
                "download_status": "done",
                "is_active": True,
                "chapter_published_at": datetime.now(UTC).isoformat(),
                "downloaded_at": None,
                "library_path": None,
                "relocation_status": "done",
                "source_chapter_name": None,
                "source_manga_title": None,
                "retry_count": 0,
            },
        ],
    }
...
async def test_apply_creates_new_comic(logged_in_client):
    ...
    await _add_source(suwayomi_source_id="src-apply-new")
    backup = {
        "version": 1,
        "sources": [
            {
                "_id": 1,
                "suwayomi_source_id": "src-apply-new",
                "name": "Src",
                "priority": 1,
                "enabled": True,
            }
        ...
        "chapter_assignments": [
            {
                ...
                "chapter_number": 1.0,
                "volume_number": None,
                "suwayomi_manga_id": "m-ap1",
                "suwayomi_chapter_id": "ch-ap1",
                "download_status": "done",
                "is_active": True,
                "chapter_published_at": datetime.now(UTC).isoformat(),
                "downloaded_at": None,
    ...
    async with database.AsyncSessionLocal() as db:
        c = (
            await db.execute(select(Comic).where(Comic.title == "Applied Comic"))
        ).scalar_one_or_none()
        assert c is not None
        aliases = (
            (await db.execute(select(ComicAlias).where(ComicAlias.comic_id == c.id)))
            .scalars()
            .all()
        )
...
async def test_apply_create_with_title_override(logged_in_client):
    ...
    backup = {
        ...
        "sources": [
            {
                "_id": 1,
                "suwayomi_source_id": "src-rename-1",
                "name": "Src",
                "priority": 1,
                "enabled": True,
            }
    ...
    zdata = _make_zip(backup)
    comic_res = [
        {"backup_id": 1, "action": "create", "title_override": "Renamed Title"}
    ]
    r = await logged_in_client.post(
        "/api/settings/import/apply",
        files={"file": ("backup.zip", zdata, "application/zip")},
    ...
    async with database.AsyncSessionLocal() as db:
        c = (
            await db.execute(select(Comic).where(Comic.title == "Renamed Title"))
        ).scalar_one_or_none()
        assert c is not None
...
async def test_apply_skip_comic(logged_in_client):
    ...
    async with database.AsyncSessionLocal() as db:
        c = (
            await db.execute(select(Comic).where(Comic.title == "Skipped Comic"))
        ).scalar_one_or_none()
        assert c is None
...
async def test_apply_merge_adds_aliases_and_pins(logged_in_client):
    ...
    await _add_source(suwayomi_source_id="src-merge-1")
    ...
    backup = {
        ...
        "sources": [
            {
                "_id": 1,
                "suwayomi_source_id": "src-merge-1",
                "name": "Src",
                "priority": 1,
                "enabled": True,
            }
        ...
        "comic_source_pins": [
            {"comic_id": 1, "source_id": 1, "suwayomi_manga_id": "m-merge-1"}
        ],
        "chapter_assignments": [
            {
                "comic_id": 1,
                ...
                "chapter_number": 5.0,
                "volume_number": None,
                "suwayomi_manga_id": "m-merge-1",
                "suwayomi_chapter_id": "ch-merge-5",
                "download_status": "done",
                "is_active": True,
                "chapter_published_at": datetime.now(UTC).isoformat(),
                "downloaded_at": None,
    ...
    async with database.AsyncSessionLocal() as db:
        aliases = (
            (
                await db.execute(
                    select(ComicAlias).where(ComicAlias.comic_id == existing.id)
                )
            )
            .scalars()
            .all()
        ...
        pins = (
            (
                await db.execute(
                    select(ComicSourcePin).where(ComicSourcePin.comic_id == existing.id)
                )
            )
            .scalars()
            .all()
...
async def test_apply_merge_skips_existing_assignments(logged_in_client):
    ...
    await _add_assignment(
        existing.id, source.id, chapter_number=1.0, suwayomi_chapter_id="ch-dup-1"
    )
        ...
        "chapter_assignments": [
            {
                ...
                "chapter_number": 1.0,
                "volume_number": None,
                "suwayomi_manga_id": "m-dup-1",
                "suwayomi_chapter_id": "ch-dup-1",
                "download_status": "done",
                "is_active": True,
                "chapter_published_at": datetime.now(UTC).isoformat(),
                "downloaded_at": None,
...
async def test_apply_source_overwrite(logged_in_client):
    ...
    async with database.AsyncSessionLocal() as db:
        s = (
            await db.execute(
                select(Source).where(Source.suwayomi_source_id == "src-ow-1")
            )
        ).scalar_one()
        assert s.priority == 5
...
async def test_apply_source_skip(logged_in_client):
    ...
    async with database.AsyncSessionLocal() as db:
        s = (
            await db.execute(
                select(Source).where(Source.suwayomi_source_id == "src-sk-1")
            )
        ).scalar_one()
        ...
        "comics": [
            {
                "_id": 1,
                "title": "Cover Comic",
                "library_title": "Cover Comic",
                "status": "tracking",
                "poll_override_days": None,
                "upgrade_override_days": None,
                "inferred_cadence_days": None,
                "created_at": None,
                "cover_file": "covers/1.jpg",
            }
        ],
        "comic_aliases": [],
        "comic_source_pins": [],
...
async def test_apply_idempotent(logged_in_client):
        ...
        "comics": [
            {
                ...
                "library_title": "Idempotent Comic",
                "status": "tracking",
                "poll_override_days": None,
                "upgrade_override_days": None,
                "inferred_cadence_days": None,
        ...
        "chapter_assignments": [
            {
                ...
                "chapter_number": 1.0,
                "volume_number": None,
                "suwayomi_manga_id": "m-idem",
                "suwayomi_chapter_id": "ch-idem-1",
                "download_status": "done",
                "is_active": True,
                "chapter_published_at": datetime.now(UTC).isoformat(),
                "downloaded_at": None,
    ...
    async with database.AsyncSessionLocal() as db:
        c = (
            await db.execute(select(Comic).where(Comic.title == "Idempotent Comic"))
        ).scalar_one()
...
async def test_apply_returns_summary(logged_in_client):
    ...
    r = await logged_in_client.post(
        ...
        data={
            "source_resolutions": "[]",
            "comic_resolutions": json.dumps([{"backup_id": 1, "action": "create"}]),
        },
    )
    assert r.status_code == 200
    result = r.json()
...
async def test_export_includes_source_overrides(logged_in_client):
    ...
    async with database.AsyncSessionLocal() as db:
        db.add(
            ComicSourceOverride(
                comic_id=comic.id, source_id=src.id, priority_override=99
            )
        )
        await db.commit()
...
async def test_apply_restores_source_overrides(logged_in_client):
    ...
    backup = {
        ...
        "sources": [
            {
                "_id": 1,
                "suwayomi_source_id": "src-ov-imp-1",
                "name": "Src",
                "priority": 1,
                "enabled": True,
            }
        ...
        "comics": [
            {
                "_id": 1,
                "title": "Override Import Comic",
                "library_title": "Override Import Comic",
                "status": "tracking",
                "poll_override_days": None,
                "upgrade_override_days": None,
                "inferred_cadence_days": None,
        ...
        "comic_source_overrides": [
            {"comic_id": 1, "source_id": 1, "priority_override": 42}
        ],
        "chapter_assignments": [],
    }
    zdata = _make_zip(backup)
            ...
            (
                await db.execute(
                    select(ComicSourceOverride).where(
                        ComicSourceOverride.comic_id == c.id
                    )
                )
            )
            .scalars()
...
async def test_apply_skips_duplicate_source_overrides(logged_in_client):
    ...
    async with database.AsyncSessionLocal() as db:
        db.add(
            ComicSourceOverride(
                comic_id=comic.id, source_id=src.id, priority_override=5
            )
        )
        await db.commit()
    ...
    backup = {
        ...
        "sources": [
            {
                "_id": 1,
                "suwayomi_source_id": "src-ov-dup-1",
                "name": "Src",
                "priority": 1,
                "enabled": True,
            }
        ...
        "comics": [
            {
                "_id": 1,
                "title": "Override Dup Comic",
                "library_title": "Override Dup Comic",
                "status": "tracking",
                "poll_override_days": None,
                "upgrade_override_days": None,
                "inferred_cadence_days": None,
        ...
        "comic_source_pins": [],
        "comic_source_overrides": [
            {"comic_id": 1, "source_id": 1, "priority_override": 99}
        ],
        "chapter_assignments": [],
    }
    zdata = _make_zip(backup)
    r = await logged_in_client.post(
        ...
        data={
            "source_resolutions": "[]",
            "comic_resolutions": json.dumps(
                [{"backup_id": 1, "action": "merge", "target_id": comic.id}]
            ),
        },
    )
    assert r.status_code == 200
